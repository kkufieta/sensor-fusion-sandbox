...
void detectObjects2(cv::Mat &img) {
  ...
  for (size_t i = 0; i < netOutput.size(); ++i) {
    float *data = (float *)netOutput[i].data; // Pointer to the data
...
int main() {
  ...
  cv::Mat img = cv::imread("../images/street.jpg");
  // img = cv::imread("../images/s_thrun.jpg");
  detectObjects2(img);
}